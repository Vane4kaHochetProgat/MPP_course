...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private fun acquire(index: Int, op: Op): AcquiredAccount? {
        ...
        while (true) {
            ...
            if (account is AcquiredAccount) {
                ...
                if (!op.completed && accounts[index].compareAndSet(account, acquiredAccount)) return acquiredAccount
            }
        }
    }
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
             ...
             * Basically, implementation of this method must perform the logic of the following code "atomically":
             */
            val reverse: Boolean = fromIndex > toIndex

            val from = acquire(if (reverse) toIndex else fromIndex, this)
            val to = acquire(if (reverse) fromIndex else toIndex, this)
            if (from != null && to != null) {
                when {
                    amount > (if (reverse) to.amount else from.amount) -> errorMessage = "Underflow"
                    (if (reverse) from.amount else to.amount) + amount > MAX_AMOUNT -> errorMessage = "Overflow"
                    else -> {
                        from.newAmount = from.amount - if (reverse) -amount else amount
                        to.newAmount = to.amount + if (reverse) -amount else amount
                    }
                }
                completed = true
            }
            release(if (reverse) fromIndex else toIndex, this)
            release(if (reverse) toIndex else fromIndex, this)
        }
    }
}